...
function uniquePack(varr) {
  ...
  for (let i=0; i < arr.length; i++) {
    for (key in arr[i]['stat']) {
      ...
      summ += (parseFloat(arr[i]['stat'][key]['s'])/100*parseFloat(price_obj[arr[i]['stat'][key]['package_id']]));
      // if (parseFloat(price_obj[arr[i]['stat'][key]['package_id']]) == NaN) {
      //   console.log(arr[i]['stat'][key]['package_id']);
      // }
      console.log(parseFloat(price_obj[arr[i]['stat'][key]['package_id']]), arr[i]['date'], arr[i]['stat'][key]['package_id']);
    }
    amount_money.push(parseFloat(summ));
    summ = 0;
  }
  // console.log(numPack);
